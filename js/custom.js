...
function cycleStart() {
	...
	if ($sliderdisable == 1){$duration = 0; }else{if ($sliderspeed != "" && $sliderdisable == ''){$duration = $sliderspeed}else{$duration = 7000;}}
		...
		if(!hasError) {
			...
			jQuery.post(jQuery('#submitUrlWidget').val(),formInput, function(data){
				...
				jQuery('form#contactFormWidget').fadeOut('fast', function() {		   
					jQuery(this).before('<p class="thanks"><strong>Thank you for your email!</strong> Your message was successfully sent and our support staff will contact you shortly</p>');
					});
				});
			});
		}
